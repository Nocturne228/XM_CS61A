...
def count_dollars_upward(total):
    ...
    def _count(total, bill):
        ...
        if bill is None:
            return 0
        ways_use_current_bill = _count(total - bill, bill)
        ways_use_bigger_bill = _count(total, next_larger_dollar(bill))
        return ways_use_current_bill + ways_use_bigger_bill
    return _count(total, 1)
...
def make_anonymous_factorial():
    ...
    #* In and out
    # (lambda f: lambda k: f(f, k)) takes f and k and return f(f, k), that is, Recursive
    # (lambda f, k: k if k == 1 else mul(k, f(f, sub(k, 1)))) lambda expr with cond expr
    # take make_anonymous_factorial(5) as example:
    # f(f, 5) -> f(f, 4) -> ... -> f(f, 1)
    # return trace: f(f, 1) = 1 -> mul(2, f(f, sub(2, 1))) -> ... -> mul(5, f(f, sub(5, 1)))
    return (lambda f: lambda k: f(f, k))(lambda f, k: k if k == 1 else mul(k, f(f, sub(k, 1))))

    #* Without helper function
    # (lambda f: f(f)) takes function f return f(f), that is, apply f to itself
